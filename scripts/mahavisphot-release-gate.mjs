...
async function main() {
  ...
  // Electron launch evidence (regenerated by the launch-evidence suite above)
  let launch = null;
  try {
    launch = JSON.parse(readFileSync(join(ROOT, "docs/mahavisphot/evidence/latest-electron-launch-evidence.json"), "utf8"));
    console.log(`      electron launch evidence: ${launch.passed}/${launch.total} checks (allPass=${launch.allPass})`);
  } catch { /* evidence not present */ }
  ...
  const report = {
    ...
    electronLaunch: launch ? { passed: launch.passed, total: launch.total, allPass: launch.allPass } : null,
